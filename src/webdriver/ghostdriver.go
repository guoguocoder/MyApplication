...
type GhostSwitches map[string]interface{}
...
func NewGhostDriver(path string) *GhostDriver {
	d := &GhostDriver{}
	d.path = path
	d.Port = 9515
	d.BaseUrl = ""
	d.Threads = 8
	d.LogPath = "ghostdriver.log"
...
func (d *GhostDriver) Start() error {
	csferr := "ghostdriver start failed: "
	if d.cmd != nil {
		return errors.New(csferr + "ghostdriver already running")
	}
	...
	switches = append(switches, "--webdriver-loglevel=ERROR")
	switches = append(switches, "--ignore-ssl-errors=true")
	if d.BaseUrl != "" {
...
func (d *GhostDriver) NewSession(desired, required Capabilities) (*Session, error) {
...
func (d *GhostDriver) Sessions() ([]Session, error) {
